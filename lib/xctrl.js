  ...
  async sum(req, res)
  {
    ...
    if (req.query && req.query._fields) {
      ...
      for (var i = 0; i < fields.length; ++i) {
        ...
        queryParams.query +
          " sum(??) as ? ";
        queryParams.params.push(fields[i]);
        queryParams.params.push(fields[i]);
      }

      queryParams.query = queryParams.query + " from ??";

      queryParams.params.push(req.app.locals._tableName);

      let myWhere = this.mysql.getWhereClause(
        req.query._where,
        req.app.locals._tableName,
        queryParams,
        " where "
      ...
      res
        ...
        .json({
          message:
            "Missing _fields in query params eg: /api/tableName/sum?_fields=numericColumn1&_where(id,in,\"1,2,3\")"
        });
    }
  }
